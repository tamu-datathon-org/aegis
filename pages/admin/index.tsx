...
function Home(): JSX.Element {
  ...
  const [currentDeadline, setCurrentDeadline] = useState(new Date()); // Fetch this from the server
  ...
  const Stats = () => {
    ...
    const numVegetarianVegan = applicants.filter((applicant) => {
      ...
      return (
        lowerRestrictions.indexOf("vegetarian") > -1 ||
        lowerRestrictions.indexOf("vegan") > -1
      );
    }).length;
    //grad students
    ...
    return (
      <div>
        <input
          ...
          style={{ marginBottom: "10px", padding: "0px", width: "350px" }}
        />
              ...
              <td>
                ...
                >
                  <button
                    ...
                    onClick={() => {
                      acceptApplicant(applicant.email);
                    }}
                  >
                    <svg
                  ...
                  <button
                    ...
                    style={{ width: "24px", height: "24px" }}
                    onClick={() => {
                      rejectApplicant(applicant.email);
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <button
                    type="button"
                    style={{ width: "24px", height: "24px" }}
                    onClick={() => deleteApplicant(applicant.email)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
  ...
  useEffect(() => {
    const fetchDeadline = async () => {
      try {
        const response = await fetch("/apply/api/admin/fetchDeadline");
        const data = await response.json();

        console.log(data);
        setCurrentDeadline(JSON.parse(data.deadline));
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const handleDeadlineChange = (e) => {
    ...
    console.log("Hello updated finish" + newDeadline);
  };

  const updateDeadline = async (deadline: Date) => {
    console.log("Hello this is the date");
    console.log(newDeadline.toLocaleString());
    try {
      await fetch("/apply/api/admin/updateDeadline", {
        method: "POST",
  ...
  const Settings = () => {
    console.log(
      currentDeadline.toLocaleString() + "I am the current deadline fucker"
    );
    ...
    return (
      <div>
        ...
        <form>
          <div>
            <label>Current Deadline: {currentDeadline.toLocaleString()}</label>
          </div>
          <div>
            <label>New Deadline:</label>
            <input
              type="date"
              value={newDeadline.toLocaleString()} // Format as 'YYYY-MM-DDTHH:mm'
              onChange={handleDeadlineChange}
            />
          </div>
          <button onClick={() => updateDeadline(newDeadline)}>
            Update Deadline
          </button>
        </form>
      </div>
    ...
    <>
      ...
      <div className="mainContent">
        ...
        <div
          ...
          style={{ alignItems: "center", height: "400px", padding: "10px" }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <button onClick={() => handleClick("applicants")}>
              Applicants
            </button>
